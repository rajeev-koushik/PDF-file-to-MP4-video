...
class DnDPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        lbl = wx.StaticText(self, label="Drag and drop the PDF file")

        submit_btn = wx.Button(self, wx.ID_ANY, "Submit")
        ...
        sizer.Add(submit_btn, 0, wx.ALL, 5)
        sizer.Add(self.fileTextCtrl, 1, wx.EXPAND|wx.ALL, 5)
        self.SetSizer(sizer)

    def submit(self, event):
        global sourcefile
        if self.fileTextCtrl.GetValue()[-4:]==".pdf":
            sourcefile=self.fileTextCtrl.GetValue()
            print(sourcefile)
            self.Destroy()
        else:
            print('Please add a PDF file only')
...
class DnDFrame(wx.Frame):
    ...
    def __init__(self):
        """Constructor"""
        wx.Frame.__init__(self, parent=None, title="G4_2019_CSE_24")
        panel = DnDPanel(self)
        self.Show()
...
for line in fr:
    ...
    for i in range(0, len(folder_names)):
        files = (fn for fn in os.listdir(picture_dir + '/' + folder_names[i]) if fn.endswith('.jpg') or fn.endswith('.JPG') or fn.endswith('.jpeg') or fn.endswith('.JPEG') or fn.endswith('.png') or fn.endswith('.PNG'))
        for file in files:
